...
function kiemTraTKDK() {
  ...
  if (!regexTKDK.test(taiKhoan)) {
    document.getElementById("errorTKDK").innerHTML =
      "Tài khoản không được chứa ký tự in hoa và ký tự đặc biệt";
    return false;
  }
  document.getElementById("errorTKDK").innerHTML = "";
  return true;
...
function kiemTraMKDK() {
  ...
  if (!regetMKDK.test(passDK)) {
    document.getElementById("errorPassDK").innerHTML =
      "Mật khẩu phải có tối thiểu 6 ký tự trừ kí tự đặc biệt";
    return false;
  }
  document.getElementById("errorPassDK").innerHTML = "";
  return true;
...
function kiemTraRePassDK() {
  ...
  if (repass != passDK) {
    document.getElementById("errorRePassDK").innerHTML =
      "Mật khẩu nhập lại phải khớp với mật khẩu ban đầu";
    return false;
  }
  document.getElementById("errorRePassDK").innerHTML = "";
  return true;
...
function taoTaiKhoan() {
  ...
  let lstTK = localStorage.getItem("dstk")
    ? JSON.parse(localStorage.getItem("dstk"))
    : [];
  let hoTen = document.getElementById("txtHoten").value;
  let taiKhoan = document.getElementById("txtTKDK").value;
  let passDK = document.getElementById("txtMKDK").value;
...
function loadThongTinDangNhap() {
  if (isDangNhap()) {
    ...
    let taga =
      `<a href="../html/thongTinKH.html" id="linkDangNhap" data-toggle="tooltip" title="Thông tin tài khoản">
      ...
      "</span>";
    let tagsp =
      '<button class= "myButton" type="button" onclick= "dangXuat()">Đăng xuất</button>';
